...
class User:
    def __init__(self, username, password, p_level, login_attempt=0, account_status="Active"):
    ...
    def __set_p_level(self, p_level):
        allowed_level = ['admin', 'standard', 'guest']
        if p_level not in allowed_level:
            raise ValueError("Privilege level does not exist.")
        self.__p_level = p_level
    ...
    def check_privileges(self, required_level):
        ...
        user_level = level.get(self.__p_level, 0) #__set_p_level(), returns Integer from level dictionary.
        required_level = level.get(required_level, 0)     # returns Integer from level dictionary.

        self.__log_record(f"Access Check: {self.username}('{self.__p_level}') attempting '{self.required_level}' task.")

        if user_level >= required_level:
            self.__log_record(f"Access GRANTED for user ('{self.username}') with '{self.__p_level}' level accessing '{self.required_level}' level.")
            return True
        
        self.__log_record(f"Access DENIED for user ('{self.username}' with '{self.__p_level}' level accessing '{self.required_level})' level.")
        return False

    def get_p_level(self):
        return self.__p_level
    
    def get_user_info(self):
        return {
            "User": self.username,
            "Role": self.get_p_level(),
